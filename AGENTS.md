...
- **Record performance.** Measure the standing metrics (theme-switch → Anki reapplied latency; add-on startup cost) and append every measurement to `docs/performance.md`, dated, with method and machine. The log is append-only; regressions get investigated, not erased. What appends is the method-consistent measurement session; automated verification-gate runs threshold-assert internally instead of appending per-run numbers — the interpretation locked with the gate (`docs/verification.md`).
