...
Never shipped. Sorted **before** ``anki_theme`` (folder name ``aa_gate_drift``),
so it loads first: it wraps ``aqt.utils.tooltip`` with a recorder (Anki Theme's
runtime imports the symbol at module import — after this — so the wrap
binds), then mutates the live ``aqt.colors`` inventory per
``ANKI_THEME_DRIFT_MODE``:

  retract   delete a real snapshot var — coverage shrank → the tooltip path
  add       add a fake var — additive churn → log-only

The recorder appends one JSON line per tooltip call to ``ANKI_THEME_DRIFT_LOG``,
...
MODE = os.environ.get("ANKI_THEME_DRIFT_MODE")
LOG = os.environ.get("ANKI_THEME_DRIFT_LOG")
...
if MODE:
    ...
    if MODE == "retract":
        ...
        ak_colors.ANKI_THEME_GATE_EXTRA = {"light": "#000000", "dark": "#000000"}
