...
| 2026-08-30 | a63e1c8 | switch-to-reapply (ticket 17 real runtime) | in-app apply **5.3 ms** (light→light Flexoki Light) / **6.2 ms** (dark→dark Gruvbox, Ethereal); end-to-end from invocation 436–463 ms; recolor completes ~2.9 s **before** `omarchy theme set` returns | `applied.jsonl` records (apply_ms, applied_at) joined to driver timings from `tests/smoke_live_switch.py` runs on a scratch Anki base; legs asserted (51 vars both slots, 3 views restyled, engine profile scripted, delivered CSS carries target `--canvas`); Ryzen 5 7500F, Arch, Anki 26.08.1, Omarchy 4.0 line | Real add-on runtime, both polarities incl. the critical same-polarity dark→dark; pixel-level rendering proof lands with tier 2 (ticket 22) per docs/verification.md |
| 2026-08-30 | a63e1c8 | add-on startup cost (ticket 17, preliminary) | startup palette apply 3.7–4.3 ms (3 scratch runs) | `applied.jsonl` startup records, same machine | Still not the standing metric — the ≥5-run profile-load delta with/without the add-on lands with ticket 19's bootloader |
